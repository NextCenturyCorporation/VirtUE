package com.ncc.savior.desktop.xpra.protocol.keyboard.dummy;
...
public class KeyMap {
    ...
    private static final Map<Integer, KeyCodeContainer> keycodesMap = new HashMap<>();

    static {

        for (KeyCode code : KeyCode.values()) {
            KeyCodeContainer container = new KeyCodeContainer();
...
//        for (KeyCode code : KeyCode.values()) {
//            if (code.isDigitKey()) {
//                String charvar = code.toString().replace("DIGIT", "");
//                if (charvar.length()==1) {
//                    map(code.ordinal(),charvar.charAt(0));
//                }
//            } else if (code.toString().length() == 1) {
//                char charvar = code.toString().charAt(0);
//                map(code.ordinal(), charvar);
//            }
//            System.out.println(code.ordinal() + " : " + Integer.toHexString(code.ordinal()).toUpperCase() + " : " + code.getName());
//        }
        //keycodesMap.put(KeyCode.BACK_SPACE.ordinal(), "BackSpace");
        //keycodesMap.put(KeyCode.ENTER.ordinal(), "Return");
		// for (Entry<Integer, KeyCodeContainer> entry : keycodesMap.entrySet()) {
		// System.out.println(entry.getKey() + " : " + entry.getValue().toString());
		// }
		// System.out.println("done");
    }

    public static void add(int ordinal, int code) {
        add(ordinal, code, "U" + Integer.toHexString(code));
    }

    public static void add(int ordinal, int code, String name) {
        KeyCodeContainer c = new KeyCodeContainer();
        c.keyCode = code;
        c.keyName = name;
        c.ordinal = ordinal;
...
//    }
    ...
    public static String getUnicodeName(int keycode) {
        ...
        KeyCodeContainer code = keycodesMap.get(keycode);
        if (code == null) {
            return null;
        }
        return code.keyName;
    ...
    public static Set<Entry<Integer, KeyCodeContainer>> getEntries() {
        return keycodesMap.entrySet();
    }

    public static int getKeyCode(int ordinal) {
        KeyCodeContainer code = keycodesMap.get(ordinal);
        if (code == null) {
            return -1;
        }
        return code.keyCode;
    ...
    public static List<List<Object>> getKeyboardList() {
        List<List<Object>> list = new ArrayList<List<Object>>();
        for (KeyCodeContainer kcc : keycodesMap.values()) {
            list.add(kcc.toList());
        }
        return list;
    }

    public static class KeyCodeContainer {
        public int ordinal;
        public int keyCode;
