package com.ncc.savior.desktop.security;
...
/**
 * Tries to obtain a new Kerberos credential based on an existing credential.
 *
 * @author clong
 *
 */
public class SignedOnUserTest {
	...
	public static void main(String[] args) {
		...
		URL loginConfigURL = SignedOnUserTest.class.getResource("login.conf");
		...
		try {
			...
			Subject.doAs(signedOnUserSubject, new PrivilegedExceptionAction<Object>() {
				...
				public Object run() throws Exception {
					GSSCredential userCredential = manager.createCredential(clientName, 8 * 3600, (Oid) krb5Oid, GSSCredential.INITIATE_ONLY);
					GSSContext context = manager.createContext(serverName, krb5Oid , userCredential, GSSContext.DEFAULT_LIFETIME);
